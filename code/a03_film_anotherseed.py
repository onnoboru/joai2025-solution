...
class joai_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_test:
            ...
            label += config.smoothing/(config.fold - 1)
...
def main():
    if config.only_infer:
        ...
        train = pd.read_csv(os.path.join(config.data_dir, "/kaggle/input/joai-dataset/train_folds_another_seed.csv"))
